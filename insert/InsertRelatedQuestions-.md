CREATE FUNCTION "InsertRelatedQuestions"(questionid INTEGER, testid INTEGER) RETURNS VOID
LANGUAGE plpgsql
AS
$$
BEGIN
...
"RelatedQuestions" AS (
    ...
    JOIN
        ...
        AND sub."questionId" NOT IN (SELECT "questionId" FROM "TestQuestion" WHERE "testId" = testId)
        AND sub."questionId" NOT IN (SELECT "relatedQuestionId" FROM "RelatedQuestion" WHERE "questionId" = "main_question_id")
        AND ((sub."questionId" < sq."main_question_id" AND sub."questionId" NOT IN (SELECT "questionId1" FROM "DuplicateQuestion" WHERE "questionId2" = sq."main_question_id"))
...
"SimilarityScores" AS (
    ...
    ORDER BY "score" DESC
    LIMIT 10 - (SELECT COUNT(*) FROM "RelatedQuestion" WHERE "questionId" = questionId AND "relatedQuestionId" NOT IN (SELECT "questionId" FROM "TestQuestion" WHERE "testId" = testId))
),
...
SET "seqId" = t."seqId" 
FROM (
    SELECT "RelatedQuestion"."id", row_number() OVER (ORDER BY similarity(q1."question", q2."question") DESC, "relatedQuestionId" ASC) AS "seqId"
    FROM "RelatedQuestion", "Question" q1, "Question" q2
    WHERE "RelatedQuestion"."questionId" = q1."id"
    AND "RelatedQuestion"."relatedQuestionId" = q2."id"
    AND "RelatedQuestion"."questionId" = questionId
) t 
WHERE t."id" = "RelatedQuestion"."id";
END
$$;

ALTER FUNCTION "InsertRelatedQuestions"(INTEGER, INTEGER) OWNER TO learner;
