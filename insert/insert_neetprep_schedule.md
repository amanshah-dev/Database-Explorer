CREATE FUNCTION insert_neetprep_schedule(
    schedule_name TEXT, 
...
BEGIN
    ...
    INSERT INTO "Schedule" ("createdAt", "updatedAt", name, description, "isActive", "isScoreBooster")
    VALUES (CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, schedule_name, schedule_description, false, true)
    ...
    FOR test IN SELECT * FROM jsonb_array_elements(tests) LOOP
        ...
        FOR topic_var_id IN SELECT * FROM jsonb_array_elements_text(test->'topicIds') LOOP
            ...
            high_yield_mcq_link := 'https://www.neetprep.com/questions/' || subject_id || '/' || topic_var_id || '?courseId=8&testId=' || test_id;
            INSERT INTO "ScheduleItemAsset" ("createdAt", "updatedAt", "assetLink", "assetName", "scheduleItemChapterId")
...
ALTER FUNCTION insert_neetprep_schedule(text, text, jsonb, jsonb) OWNER TO learner;
