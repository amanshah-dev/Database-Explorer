CREATE FUNCTION "GenerateQuestionsFromQuesBank"(testpatternid INTEGER, scaled_sections chapter_section_questions[], exceptquestionsfuncwithparams JSONB)
...
DECLARE
    ...
    test_index INTEGER;
    exclude_query TEXT;
    exclude_query_args JSONB;
    exclude_query_args_text TEXT;
    exclude_ids INTEGER[];
BEGIN
    ...
    -- Extract function name and arguments from JSONB
    SELECT exceptQuestionsFuncWithParams->>'function_name' INTO exclude_query;
    SELECT exceptQuestionsFuncWithParams->'args' INTO exclude_query_args;

    -- Construct the argument list for the function call
    exclude_query_args_text := (
        SELECT string_agg(quote_literal(value::text), ', ')
        FROM jsonb_each_text(exclude_query_args)
    );

    -- Execute the function to get the list of question IDs to exclude
    EXECUTE format('SELECT array_agg("questionId") FROM %s(%s)', exclude_query, exclude_query_args_text) INTO exclude_ids;
    ...
    FOREACH row IN ARRAY scaled_sections LOOP
            ...
            FOR selected_question IN
                ...
                WHERE "testId" = test_ids[test_index]
                    ...
                    NOT IN (
                        ...
                        UNION SELECT unnest(exclude_ids)
        ...
        IF questions_remaining > 0 THEN
            -- TODO: HANDLE THIS SCENARIO BY JUST GETTING REQUIRED NUMBER OF QUESTIONS WITHOUT ANY EXCLUDE IDS USED ABOVE
            -- TODO: CLEAN THE CODE BELOW AS WE HAVE CURRENTLY COPIED PASTED THE CODE FROM ABOVE W/O EXCLUDE IDS PART
...
ALTER FUNCTION "GenerateQuestionsFromQuesBank"(integer, chapter_section_questions[], jsonb) OWNER TO neetprep_rw;
