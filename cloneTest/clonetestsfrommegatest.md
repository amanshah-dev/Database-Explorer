...
create function clonetestsfrommegatest(maintestid integer) returns void
    language plpgsql
as
$$
DECLARE
    sectionRecord RECORD;            -- Variable to hold section data
    newTestId INTEGER;               -- Variable to store the new test ID
    newTestQuestions INTEGER[];      -- Variable to store questions for the new test
BEGIN
    -- Loop over each ChapterSection entry to clone tests
    FOR sectionRecord IN SELECT * FROM "ChapterSection" LOOP
        -- Create a new test for the current chapter and section
        INSERT INTO "Test" ("name", "description", "instructions", "numQuestions", "durationInMin", "free", "sections", "positiveMarks", "negativeMarks")
        VALUES (
        ...
        VALUES (newTestId, sectionRecord."chapterId", sectionRecord."section", 10000, 1);
    END LOOP;
END;
$$;

-- Alter function owner
alter function clonetestsfrommegatest(integer) owner to neetprep_rw;
