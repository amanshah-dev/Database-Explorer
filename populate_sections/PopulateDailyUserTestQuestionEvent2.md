create function "PopulateDailyUserTestQuestionEvent"(testids text, days integer DEFAULT 1, startdate date DEFAULT '2023-01-04'::date) returns void
    language plpgsql
as
$$
DECLARE
...
BEGIN
    ...
    SELECT max("eventDate") from "DailyUserTestEvent" where "event" = 'Question' into maxDate;
    ...
    select "userId", "testId", 'Question', startDate, 
...
alter function "PopulateDailyUserTestQuestionEvent"(text, integer, date) owner to learner;
