-- Create function to add Diwali course offer for Offline Test Series Course
create function "AddDiwaliCourseOfferForOfflineTestSeriesCourse"(totalamount integer) returns void
    language plpgsql
as
$$
DECLARE
...
for t_row in
    ...
    values(coalesce("t_row"."email", ''), coalesce("t_row"."phone", ''), 'Classroom Test Series offer: Target Batch Students', '<p class="course-offer-discount">Enroll in Offline Classroom test series. The Diwali discount is based on your previous purchases and valid only for 5 days. You will be allotted the center of your choice from <a href="https://www.neetprep.com/exam-info/Offline-Test-Series-Centers" target="_blank">the list of available centers</a> after payment.</p>', 3323, 9999, totalAmount - "t_row"."amount", '2024-05-10', current_date, current_timestamp + interval '5 days', true) ON CONFLICT ON CONSTRAINT single_applicable_course_offer do nothing;
end loop;
END
$$;

-- Alter function owner
alter function "AddDiwaliCourseOfferForOfflineTestSeriesCourse"(integer) owner to learner;
