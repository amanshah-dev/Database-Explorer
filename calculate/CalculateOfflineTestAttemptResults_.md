-- Create function to calculate results for offline test attempts based on testId and offlineTestAttemptId
create function "CalculateOfflineTestAttemptResults"(testid integer, offlinetestattemptid integer) returns void
    language plpgsql
as
$$
BEGIN
  ...
  -- Update the answers for OfflineTestAttempt
  UPDATE "OfflineTestAttempt" 
  SET "quesAnswers" = t."jsonAnswers" 
  FROM (
    SELECT
      "OfflineTestAttempt"."id" AS "offlineTestAttemptId",
    ...
    FROM
      ...
      "TestQueWSeq"
    WHERE
      "TestQueWSeq"."testId" = "OfflineTestAttempt"."rawTestId" 
      AND ordinality = "TestQueWSeq"."seqNo" 
      AND value::text IN ('A', 'B', 'C', 'D') 
    GROUP BY "OfflineTestAttempt"."id"
  ) t 
  WHERE t."offlineTestAttemptId" = "OfflineTestAttempt"."id" 
    AND "OfflineTestAttempt"."id" = offlinetestattemptid;
  
  -- Update the results for OfflineTestAttempt
  UPDATE "OfflineTestAttempt" 
    ...
    FROM (
      ...
      FROM (
        SELECT 
          ...
          COUNT(t1."queAnswer") OVER(PARTITION BY t.section, "OfflineTestAttempt"."id" ORDER BY t."questionNum") AS "attemptCount", 
          CASE 
            WHEN COUNT(t1."queAnswer") OVER(PARTITION BY t.section, "OfflineTestAttempt"."id" ORDER BY t."questionNum") <= COALESCE(t."attemptLimit"::INTEGER, t."endNum"::INTEGER - t."startNum"::INTEGER + 1) 
            THEN CASE 
      ...
      WHERE 
        ...
        AND "OfflineTestAttempt"."rawTestId" = t."testId" 
  ...
  WHERE t4."offlineTestAttemptId" = "OfflineTestAttempt"."id" 
    AND "OfflineTestAttempt"."id" = offlinetestattemptid;
END
$$;

-- Alter function owner
alter function "CalculateOfflineTestAttemptResults"(integer, integer) owner to learner;
