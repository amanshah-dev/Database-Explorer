create procedure update_topscorer(IN achievementcategoryid bigint, IN userids bigint[] DEFAULT ARRAY[]::bigint[])
    language plpgsql
as
$$
DECLARE
...
BEGIN
    FOR achievement_record IN
        ...
        LOOP
            eligibleUsers := bestScorers(userIds, achievement_record.current_level);
            call insert_user_achievements(eligibleUsers, achievement_record);
        END LOOP;
END;
$$;

alter procedure update_topscorer(bigint, bigint[]) owner to neetprep_rw;
