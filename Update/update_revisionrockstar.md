create procedure update_revisionrockstar(IN achievementcategoryid bigint, IN userids bigint[] DEFAULT ARRAY[]::bigint[])
    language plpgsql
as
$$
DECLARE
...
BEGIN
    FOR achievement_record IN
        ...
        LOOP
            eligibleUsers := dppStreakAchievers(userIds, achievement_record.current_level, 'revision'::dpp_type_enum);
            call insert_user_achievements(eligibleUsers, achievement_record);
        END LOOP;
END;
$$;

alter procedure update_revisionrockstar(bigint, bigint[]) owner to neetprep_rw;
