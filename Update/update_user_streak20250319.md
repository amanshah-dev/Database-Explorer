create function update_user_streak20250319(p_user_id integer, p_event_count integer) returns integer
    language plpgsql
as
$$
DECLARE
    ...
    v_user_created_date DATE;  -- The date when the user was created
...
BEGIN
    ...
    SELECT "lastStreakUpdated", COALESCE("streakCount", 0)
    ...
    ELSE
       ...
       SELECT CURRENT_DATE::TIMESTAMP AT TIME ZONE 'Asia/Kolkata' + interval '1 day' into dateUpper;
        ...
        IF v_last_streak_date = CURRENT_DATE - INTERVAL '1 day' THEN
        ...
        ELSE
            ...
            IF v_last_streak_date IS NULL THEN
                ...
                v_check_date := CURRENT_DATE - INTERVAL '2 years';
                v_last_streak_date := CURRENT_DATE - INTERVAL '2 years';
        ...
        IF v_last_missing_date = v_check_date THEN
            IF v_has_activity_today THEN
                ...
                v_streak := v_streak + (CURRENT_DATE - v_last_missing_date);
                v_last_streak_update := CURRENT_DATE;  -- Set last updated date to today
            ...
            ELSE
                ...
                v_streak := v_streak + (CURRENT_DATE - v_last_missing_date) - 1;
                v_last_streak_update := CURRENT_DATE - INTERVAL '1 day';  -- Set last updated date to yesterday
        ...
        IF EXISTS (SELECT 1 FROM "UserStreakData" WHERE "userId" = p_user_id) THEN
            IF v_last_streak_date = CURRENT_DATE - INTERVAL '1 day' AND NOT v_has_activity_today THEN
        ...
        ELSE
            ...
            INSERT INTO "UserStreakData" ("userId", "streakCount", "lastStreakUpdated", "createdAt", "updatedAt")
            VALUES (p_user_id, v_streak, v_last_streak_update, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
alter function update_user_streak20250319(integer, integer) owner to learner;
